...
class DonationsManagement:
    ...
    @classmethod
    def update_donation(cls, donation_id: int, donating_user_id: int, availability_status_str: str = None, category_str: str = None, description: str = None, address: str = None, available_until_str: str = None):
        ...
        update_sql += "  WHERE id = %s AND donating_user = %s;"
        args.extend([donation_id, donating_user_id])

        num_rows_affected = dbManager.commit(update_sql, args)
        if num_rows_affected == dbManager.ERROR_CODE:
            raise app_errors.AppError('Failed updating donation', payload={'donation_id': donation_id, 'user_id': donating_user_id})
